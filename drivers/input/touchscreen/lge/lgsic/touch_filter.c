...
struct timespec64 ex_debug[EX_PROFILE_MAX];
...
bool chk_time_interval(struct timespec64 t_aft, struct timespec64 t_bef, int t_val)
{
	int64_t interval = t_val * 1000000ull;
	if (t_aft.tv_sec - t_bef.tv_sec == 0) {
		if ((time_interval(t_aft.tv_nsec, t_bef.tv_nsec)) <= interval)
			return true;
	} else if (t_aft.tv_sec - t_bef.tv_sec == 1) {
		if (t_aft.tv_nsec + 1000000000ull - t_bef.tv_nsec <= interval)
			return true;
	}
...
int debug_detect_filter(struct device *dev, int count)
{
	...
	if (prev_touch.count < count) {
		ktime_get_ts64(&ex_debug[DEBUG_CURR_P]);
	...
	/* minus count senario */
		...
		ktime_get_ts64(&ex_debug[DEBUG_CURR_R]);
