...
static void write_file(struct device *dev, char *data, int write_time)
{
	...
	if (fname) {
		fd = vfs_open(fname, O_WRONLY|O_CREAT|O_APPEND, 0666);
		vfs_chmod(fname, 0666);
	} else {
		TOUCH_E("%s : fname is NULL, can not open FILE\n", __func__);
		set_fs(old_fs);
		return;
	...
	if (fd >= 0) {
		if (write_time == TIME_INFO_WRITE) {
			...
			vfs_write(fd, time_string, strlen(time_string), 0);
		}
		vfs_write(fd, data, strlen(data), 0);
		vfs_close(fd);
	} else {
...
static int sdcard_spec_file_read(struct device *dev)
{
	...
	if(touch_boot_mode_check(dev) >= MINIOS_MFTS_FOLDER)
		...
		path_idx = 0;
	set_fs(KERNEL_DS);
	fd = vfs_open(path[path_idx], O_RDONLY, 0);
	if (fd >= 0) {
		vfs_read(fd, line, sizeof(line), 0);
		vfs_close(fd);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	trim_ret = true; //lge_is_valid_U2_FTRIM_reg();
